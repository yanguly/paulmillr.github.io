...
function detectType(str) {
    let type = 'numeric';
    for (let x of str) {
        if (info$1.alphabet.numeric.has(x))
            continue;
        type = 'alphanumeric';
        if (!info$1.alphabet.alphanumerc.has(x))
            return 'byte';
    }
    return type;
}
...
function encode(ver, ecc, data, type) {
    ...
    if (type === 'numeric') {
        const t = info$1.alphabet.numeric.decode(data.split(''));
        const n = t.length;
        for (let i = 0; i < n - 2; i += 3)
            encoded += bin$1(t[i] * 100 + t[i + 1] * 10 + t[i + 2], 10);
        if (n % 3 === 1) {
            encoded += bin$1(t[n - 1], 4);
        }
        else if (n % 3 === 2) {
            encoded += bin$1(t[n - 2] * 10 + t[n - 1], 7);
        }
    }
    else if (type === 'alphanumeric') {
        const t = info$1.alphabet.alphanumerc.decode(data.split(''));
        const n = t.length;
        for (let i = 0; i < n - 1; i += 2)
            encoded += bin$1(t[i] * 45 + t[i + 1], 11);
        if (n % 2 == 1)
            encoded += bin$1(t[n - 1], 6); // pad if odd number of chars
    }
    else if (type === 'byte') {
    ...
    const { capacity } = info$1.capacity(ver, ecc);
    const len = bin$1(dataLen, info$1.lengthBits(ver, type));
    let bits = info$1.modeBits[type] + len + encoded;
    ...
    return interleave$1(ver, ecc).encode(bytes);
}
// DRAW
function drawQR(ver, ecc, data, maskIdx, test = false) {
    const b = drawTemplate$1(ver, ecc, maskIdx, test);
    let i = 0;
    const need = 8 * data.length;
    zigzag$1(b, maskIdx, (x, y, mask) => {
        let value = false;
        if (i < need) {
...
function main() {
  ...
  const qrGifDataUrl = (text) => {
    const gifBytes = createQR(text, 'gif', {
      scale: 7,
    });
