<script>
  ...
  import { sha3_512, keccak_512 } from '@noble/hashes/sha3';
  ...
  // public key
  $: {
    const hex = P521.getPublicKey(privKey);
    const pub = P521.ProjectivePoint.fromHex(hex);
  ...
  $: {
    if (isMessage) {
      ...
      const sig = P521.sign(msgHash, privKey);
...
<div class="curve-data">
  <h3 class="curve-data__header">Public key</h3>
